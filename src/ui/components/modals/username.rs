...
pub fn render_username_modal(
    ...
    render_modal_frame(f, area, &config, |f, inner_area| {
        render_username_content(f, inner_area, username, hide_username, input_mode);
    });
}

fn render_username_content(
    f: &mut Frame,
    area: Rect,
    username: &str,
    hide_username: bool,
    ...
    let toggle_instruction_area = Layout::default()
        .direction(Direction::Horizontal)
        .horizontal_margin(3)
        .constraints([Constraint::Min(0)])
        .split(content_chunks[3])[0];

    let toggle_instruction = Paragraph::new("Press 'tab' to toggle:")
        .style(Style::default().fg(Color::Gray))
        .alignment(Alignment::Left);
    f.render_widget(toggle_instruction, toggle_instruction_area);

    let toggle_area = Layout::default()
        .direction(Direction::Horizontal)
        .horizontal_margin(3)
        .constraints([Constraint::Min(0)])
        .split(content_chunks[4])[0];

    let checkbox = if hide_username { "[✓]" } else { "[ ]" };
    let toggle_text = format!("{checkbox} Hide username in header");
    let toggle_widget = Paragraph::new(toggle_text)
        .style(Style::default().fg(Color::White))
        .alignment(Alignment::Left);
    ...
    f.render_widget(help_text, content_chunks[6]);
